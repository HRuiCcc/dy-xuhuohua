...
**Q3：同步联系人结果为 0？**
先确认「凭证」页显示已登录。已登录仍为 0 时，程序会自动尝试打开抖音的「读屏标签」（火花信息在标签里最全）并兼容多种页面形态：`火花 N天` 文字、`N天` 文字、读屏标签 aria、以及只有火花图标 + 裸天数数字的版本（Docker/新版页面常见）。仍为 0 时大概率是抖音改版，请把「日志」页的「页面诊断片段」反馈给开发者。
